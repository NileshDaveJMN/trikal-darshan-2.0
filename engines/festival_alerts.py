...
def get_tithi_at_hour(date_obj, hour_float):
    ...
    tithi_idx = int(((moon[0] - sun[0]) % 360) / 12.0)
...
def get_today_solar_festivals(date_obj=None):
    ...
    for f in SOLAR_FESTIVALS:
        if f["day"] == date_obj.day and f["month"] == date_obj.month:
            matched.append(f)
    return matched
...
def get_today_festivals(panchang=None):
    ...
    for rule in FESTIVAL_RULES:
        ...
        if time_slot and time_slot in slot_hours:
            ...
            if rule["tithi"] == calc_tithi and rule["paksha"] == calc_paksha:
                festivals.append(rule)
        else:
            if rule["tithi"] == p_tithi and rule["paksha"] == p_paksha:
                festivals.append(rule)

    solar = get_today_solar_festivals(current_date)
    return festivals + solar
...
आज का पर्व: {festival_name} ({festival_desc})
देवता: {festival_deity} | समुदाय: {community}
...
उपयोगकर्ता की जन्म-कुंडली:
  ...
  गुरु: {guru} | शुक्र: {shukra} | शनि: {shani}
...
def process_user_festival(profile, kundali, natal_pos, lagna_idx, dasha, festival):
    ...
    if not ai_msg:
        ...
        if community == "National":
            ai_msg = f"{user_name} जी, आज {emoji} {festival_name} है। जय हिंद! 🇮🇳"
        elif community in ["Sikh", "Jain", "Buddhist", "Christian"]:
            ai_msg = f"{user_name} जी, {emoji} {festival_name} की हार्दिक शुभकामनाएं! इस पावन अवसर पर आपके जीवन में सुख-शांति आए। 🙏"
        else:
            deity = festival.get("deity", "भगवान")
            ai_msg = f"{user_name} जी, आज {festival_name} पर {deity} की विशेष पूजा करें। यह दिन आपके लिए शुभ फलदायी हो। 🙏"
    ...
    print(f"     ✅ {user_name}: {festival_name} का अलर्ट इनबॉक्स में सेव हुआ।")
    return True